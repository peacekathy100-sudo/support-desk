...
@section('content')
<div style="display: flex; height: calc(100vh - 100px); gap: 0; background: #fff;">
    ...
    ">
        ...
        <div style="padding: 1.5rem; border-bottom: 1px solid #e0e0e0;">
            <h2 style="margin: 0 0 1rem 0; font-size: 1.5rem; font-weight: 700; color: #000;">Support Chat</h2>
            <form method="GET" action="{{ route('admin.chat.index') }}" style="display: flex; gap: 0.5rem;">
                ...
                <button type="submit" style="
                    ...
                    font-weight: 500;
                    transition: all 0.2s;
                " onmouseover="this.style.background='#2980BB'" onmouseout="this.style.background='#3496D7'">
        ...
        ">
            @forelse($conversations as $conversation)
                @php
                    $participant = $conversation->participants->firstWhere('participantable_type', '!=', App\Models\SysUser::class);
                    $clientName = $participant?->participantable->full_name ?? 'Client';
                    $lastMessage = $conversation->latestMessage;
                    $initials = strtoupper(substr($clientName, 0, 2));
                @endphp
                <a href="{{ route('admin.chat.show', $conversation) }}" style="
                    display: flex;
                    align-items: center;
            ...
            @empty
                ...
                ">
                    ...
                    <p style="margin: 0; font-size: 1rem; font-weight: 500;">No conversations</p>
                </div>
            @endforelse
        </div>
        ...
        <div style="
            ...
            text-align: center;
        ">
            Page {{ $conversations->currentPage() }} of {{ $conversations->lastPage() }}
        </div>
        @endif
    </div>
    ...
    ">
        <div style="text-align: center;">
            ...
            <p style="font-size: 1.1rem; font-weight: 500; margin: 0;">Select a conversation to start</p>
            <p style="margin: 0.5rem 0 0; font-size: 0.95rem;">Click on a conversation from the left sidebar</p>
        </div>
    </div>
</div>
