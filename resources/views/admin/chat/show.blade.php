...
@php
    ...
    $clientParticipant = $conversation->participants->firstWhere('participantable_type', '!=', SysUser::class);
    $clientName = $clientParticipant?->participantable->full_name ?? 'Client';
    $clientEmail = $clientParticipant?->participantable->email ?? 'No email';
@endphp
...
<div style="display: flex; height: calc(100vh - 100px); gap: 0;">
    ...
    ">
        ...
        <div style="padding: 1.5rem; border-bottom: 1px solid #e0e0e0;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                <h2 style="margin: 0; font-size: 1.5rem; font-weight: 700; color: #000;">Support Chat</h2>
                <a href="{{ route('admin.chat.index') }}" style="
                    width: 40px;
                    height: 40px;
                    border-radius: 50%;
                    background: #f0f0f0;
            ...
            <form method="GET" action="{{ route('admin.chat.index') }}" style="display: flex; gap: 0.5rem;">
                <input 
                    type="text" 
                    name="q" 
                    placeholder="Search..." 
        ...
        ">
            @forelse($conversations as $conv)
                @php
                    $participant = $conv->participants->firstWhere('participantable_type', '!=', SysUser::class);
                    $name = $participant?->participantable->full_name ?? 'Client';
                    $isActive = $conv->id === $conversation->id;
                    $initials = strtoupper(substr($name, 0, 2));
                @endphp
                <a href="{{ route('admin.chat.show', $conv) }}" style="
                    display: flex;
                    align-items: center;
    ...
    ">
        ...
        ">
            <div>
                <h3 style="margin: 0 0 0.3rem 0; font-size: 1.1rem; font-weight: 600;">{{ $clientName }}</h3>
                <p style="margin: 0; font-size: 0.9rem; opacity: 0.9;">{{ $clientEmail }}</p>
            </div>
            <div style="display: flex; gap: 0.5rem;">
                @if(!$conversation->is_archived)
                    <form action="{{ route('admin.chat.archive', $conversation) }}" method="POST" style="margin: 0;">
                        @csrf
        ...
        ">
            @forelse($conversation->messages as $message)
                @php $isMine = $message->sender_type === SysUser::class; @endphp
                <div style="
                    display: flex; 
                ...
                " class="message-row">
                    @if(!$isMine)
                        <img src="https://via.placeholder.com/32?text={{ substr($clientName, 0, 1) }}" style="
                            width: 32px;
                            height: 32px;
                            border-radius: 50%;
                            flex-shrink: 0;
                    ...
                    ">
                        @if(!$isMine)
                            <span style="font-size: 0.75rem; font-weight: 600; padding: 0 0.5rem; color: #666;">{{ $clientName }}</span>
                        @endif
                        <div style="
                            max-width: 65%;
                            padding: 0.7rem 1rem;
            ...
            @empty
                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; color: #999;">
                    ...
                    <p style="margin: 0.5rem 0 0; font-size: 0.9rem; opacity: 0.7;">Start the conversation by sending a message</p>
                </div>
            @endforelse
        </div>
        ...
        <div id="typing-indicator" class="small text-muted d-none" style="padding: 0.5rem 1.5rem; color: #999; font-size: 0.9rem; display: flex; gap: 0.5rem; align-items: center;">
            ...
            <span id="typing-name">{{ $clientName }} is typing</span>
        </div>
...
<script>
    // ================================================================
    // BULLETPROOF MESSAGE SENDER - Admin Debug + Error Handling
    // ================================================================
    ...
    window.sendChatMessage = function() {
        try {
            console.log('🔧 sendChatMessage called (admin)');
            ...
            // Build exact URL - ADMIN version
            const url = `/admin/chat/${conversationId}/message`;
    ...
    window.chatTypingUrl = "{{ route('admin.chat.typing', $conversation) }}";
    window.setChatConfig({
        conversationId: "{{ $conversation->id }}",
        currentUserId: "{{ auth()->user()->user_id }}",
        senderType: "{{ addslashes(SysUser::class) }}",
        typingUrl: window.chatTypingUrl,
        otherName: "{{ addslashes($clientName) }}",
    });
    
    console.log('✅ All admin chat handlers initialized');
</script>
@endpush
@endsection
